# file: yushu_wanwu_agent.py
# description: 语枢·万物 Thinker Agent v1.1 —— 数据分析与逻辑推理（ReAct-C Step4）
# author: YanYuCloudCube Team <admin@0379.email>
# created: 2026-09-25
# status: active
# tags: [agent],[thinker],[data-analysis]

"""语枢·万物：思考者·数据分析（业务执行层）。

能力：数据分析 / 业务逻辑推理 / 问题分解 / 结论论证验证。
模型映射：nemotron-3-super-120b-a12b（节点1，共享算力池，1M 上下文）。
"""
...
原则：不夸大数据精度，区分事实与推断，诚实面对数据缺口。""",
    ...
    def analyze(self, query: str, knowledge_context: list = None) -> str:
        ...
        :param knowledge_context: RAG 注入的带来源知识片段
        :return: 四段式分析报告
        """
        context = ""
        if knowledge_context:
            context = "参考知识库信息（引用时保留来源标识）：\n" + "\n".join(
                f"- {k}" for k in knowledge_context
            )
...
数据不足的部分明确说明，不得虚构数值。"""
        ...
        logger.info("[%s] 正在执行数据分析：%s...", self.name, query[:50])
